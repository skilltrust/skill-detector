...
func TestAllowedDomainsArePerCommandDeclarations(t *testing.T) {
	for _, tc := range []struct {
		...
		{"dynamic-userinfo", `{"name":"Bash","input":{"command":"curl \"https://$PREFIX@api.example.com/a\"","allowed_domains":["api.example.com:443"]}}`, "cannot be compared"},
		{"static-userinfo-unsupported", `{"name":"Bash","input":{"command":"curl \"https://user@api.example.com/a\"","allowed_domains":["api.example.com:443"]}}`, "cannot be compared"},
		{"powershell-continuation", `{"name":"PowerShell","input":{"command":"Invoke-WebRequest ${PREFIX}` + "`" + `\nhttps://api.example.com/a","allowed_domains":["api.example.com:443"]}}`, "cannot be compared"},
		{"unterminated-quote", `{"name":"Bash","input":{"command":"curl \"https://api.example.com/a","allowed_domains":["api.example.com:443"]}}`, "cannot be compared"},
		...
		{"unquoted-glob", `{"name":"Bash","input":{"command":"curl https://api.example.com/a *.example.com","allowed_domains":["api.example.com:443"]}}`, "cannot be compared"},
...
func TestClaudeSettingsAllowIgnoredLargeJSONNumber(t *testing.T) {
	content := []byte(`{"extensionData":{"large":1e1000},"permissions":{"deny":["Read(**/.env)"]}}`)
	ctx := model.FileContext{Path: ".claude/settings.json"}
	if _, err := ClaudeConfigurationDiagnostics(content, ctx); err != nil {
		t.Fatal(err)
	}
}
