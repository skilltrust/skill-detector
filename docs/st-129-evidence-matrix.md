...
| DHOOK | [Claude hooks reference](https://code.claude.com/docs/en/hooks) | current docs, observed 2026-09-24 |
| DMCP | [Claude managed MCP reference](https://code.claude.com/docs/en/managed-mcp) | current docs, observed 2026-09-23 |
| DGATE | [Claude apps gateway configuration](https://code.claude.com/docs/en/claude-apps-gateway-config) | current docs, observed 2026-09-24 |
...
|---|---|---|---|
| 01.1 Inline `!` commands | C271: at v2.1.271 auto-mode skill/slash inline commands use default-mode permissions; undecided commands become reviewed tool calls. Claude provider; skill/command candidate source; trust/session unknown. Older range unknown. | Parsed inline executable syntax. Risky: undecided `!` command in `testdata/malicious/claude-permission-context`; benign: literal prose and inactive parsed frontmatter in the paired clean fixture. | Conditional review warning; never execute. `TestInlineShellDistinguishesExecutableSyntax` passed: inline/fenced executable forms differ from prose, escaped and assignment forms, and parsed frontmatter metadata. |
| 01.2 Per-command domains | C271: `allowed_domains` for Bash, PowerShell and Monitor in auto mode with sandboxing, scoped to one command. Project declaration is candidate only; enforcement/session unknown. | Risky: broader/different destination in the malicious fixture. Benign: narrow destination in the paired clean fixture; absence control: unavailable context. | Declaration inventory without confinement claim. `TestAllowedDomainsArePerCommandDeclarations` passed for narrow, wildcard-broad, different, duplicate, port-specific, IPv6, unsupported and missing-command contexts across all three tools. |
...
| 06.4b Claude `SubagentStop` lifecycle (ST-137) | C275: a specific matcher no longer fires for a stopping subagent with empty agent type. Claude provider; project settings candidate; trust/session unknown unless supplied. Other versions, including prereleases, are unresolved. | Supported nested hook schema with supplied event/agent-type context. Risky: matching nonempty type. Benign: nonmatching and empty type; unknown context control. | `TestSubagentStopEmptyAgentTypeRequiresExactAnchor` passed: only exact 2.1.275 certifies the specific empty-type nonmatch; older, unknown and prerelease versions remain unresolved. Catch-all empty and unknown matcher input stay distinct. |
...
| Row | Evidence and context | Supported input; inert controls | Expected diagnostic; engine/test result |
|---|---|---|---|
| 07.1 Gateway declarations | C277 adds `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1` and static upstream `headers`; DGATE documents current gateway shape. Claude gateway provider; submitted wrapper/config source is candidate only; runtime topology unknown. YAML aliases are followed with bounded traversal. | Supported exact environment assignment and YAML/JSON declaration plus upstream header maps. Risky: explicit boundary claim treated as proof in `testdata/malicious/claude-hook-gateway-context`. Benign: absent claim and static/placeholder headers in the paired clean fixture. | `TestGatewayBoundaryDeclarationNeedsTopology` passed: inventories the claim and header counts without values, activation or maliciousness claims, including alias-declared fields. |
| 07.2 Sole-egress precondition | C277 states the flag is for gateways whose only egress is a forward proxy and changes hostname resolution behavior. DNS/routing remain unavailable to static scan. | Risky: claim with unknown topology. Benign: supplied sole-egress condition and absent declaration control. No proxy or host is contacted. | `TestGatewayBoundaryDeclarationNeedsTopology` passed for unknown and supplied sole-egress context; both retain the DNS/routing and confinement limitation. |
| 07.3 Secret-safe output | DHOOK/DGATE describe header values; ST-128 owns shared hosted redaction, not detector output. | Risky: synthetic URL userinfo/query in handler URLs, command strings and allowlist arrays; URL-like header/response values; literal gateway headers; non-HTTP credential URLs; free-text handler fields (`prompt`, `args`, `input`, `statusMessage`), `allowedEnvVars` and `httpHookAllowedEnvVars` names, `$NAME` tokens in structural `if`, YAML duplicate `type` keys, and credential-shaped hook command tokens (`-u`/`--user=`/`-H` values, `AKIA`). Benign: static/placeholder headers and empty header maps retain ordinary diagnostics. | `TestConfigurationSanitizerRedactsHookAndGatewaySecrets`, `TestHookGatewayDiagnosticsAndSecretsSurviveFullScannerPath` and `TestHookFreeTextAndEnvNamesStayOutOfOutput` passed: findings, warnings, serialized result, permissions, verifier findings and verifier-bound `FileContext` omit synthetic values while retaining destinations and generic SD-007. |
...
| 08.3 Hook context (ST-137) | DHOOK documents command/HTTP handlers, exact/list/unanchored-regexp matchers, events without matcher support, POST event data, merged URL allowlists and two-level env/header exposure. Comma-list semantics require 2.1.191; hyphen-exact semantics require 2.1.195. Claude provider; settings source is candidate and trust/session/version remain unknown unless supplied. | Supported nested command/HTTP schema. Risky: matching HTTP hook with synthetic exposure in the malicious fixture. Benign: exact nonmatch, list/regexp match, ignored event matcher, unknown event or matcher, unsupported HTTP event, harmless external URL/header placeholders and legacy flat shape. | `TestHookTypeEventMatcherAndExposure`, `TestHookMatcherSemantics` and `TestHookGatewayDiagnosticsAndSecretsSurviveFullScannerPath` passed: diagnostics preserve type/event/matcher, destination and exposure; version-dependent or missing context never implies activation, legacy/unsupported shapes are not certified, generic SD-007 remains, no host is contacted and response data cannot alter analysis. |
...
The original ST-135 contract tests passed on 2026-09-23. The completed ST-136
and ST-137 rows, including their full scanner-path tests, passed under
`go test ./...` on 2026-09-24; `golangci-lint run` reported zero issues. Rows
still marked future remain requirements until their owners add actual results.
