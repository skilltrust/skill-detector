...
| 07.3 Secret-safe output | DHOOK/DGATE describe header values; ST-128 owns shared hosted redaction, not detector output. | Risky: synthetic URL userinfo/query in handler URLs, command strings and allowlist arrays; URL-like header/response values; literal gateway headers. Benign: static/placeholder headers in the clean fixture. | `TestConfigurationSanitizerRedactsHookAndGatewaySecrets` and `TestHookGatewayDiagnosticsAndSecretsSurviveFullScannerPath` passed: findings, warnings, serialized result, verifier findings and verifier-bound `FileContext` omit synthetic values while retaining destinations and generic SD-007. |
