...
| 06.4b Claude `SubagentStop` lifecycle (ST-137) | C275: a specific matcher no longer fires for a stopping subagent with empty agent type. Claude provider; project settings candidate; trust/session unknown unless supplied. Older behavior is unknown. | Supported nested hook schema with supplied event/agent-type context. Risky: matching nonempty type. Benign: nonmatching and empty type; unknown context control. | `TestSubagentStopEmptyAgentTypeDoesNotMatch` passed: specific nonempty, specific empty, catch-all empty and unknown matcher input remain distinct; activation is never inferred from file presence. |
...
| 07.3 Secret-safe output | DHOOK/DGATE describe header values; ST-128 owns shared hosted redaction, not detector output. | Risky: synthetic URL userinfo/query and literal headers in the malicious fixture. Benign: static/placeholder headers in the clean fixture. | `TestConfigurationSanitizerRedactsHookAndGatewaySecrets` and `TestHookGatewayDiagnosticsAndSecretsSurviveFullScannerPath` passed: findings, warnings, serialized result and verifier-bound `FileContext` omit synthetic values while retaining destinations and generic SD-007. |
...
| 08.3 Hook context (ST-137) | DHOOK documents command/HTTP handlers, event matchers, POST event data, merged URL allowlists and two-level env/header exposure. Current docs do not establish historical ranges. Claude provider; settings source is candidate and trust/session/version remain unknown unless supplied. | Supported nested command/HTTP schema. Risky: matching HTTP hook with synthetic exposure in the malicious fixture. Benign: command/nonmatching/unknown event or matcher, unsupported HTTP event, harmless external URL/header placeholders and legacy flat shape in the paired fixture/tests. | `TestHookTypeEventMatcherAndExposure` and `TestHookGatewayDiagnosticsAndSecretsSurviveFullScannerPath` passed: diagnostics preserve type/event/matcher, destination and exposure; missing context never implies activation, legacy/unsupported shapes are not certified, generic SD-007 remains, no host is contacted and response data cannot alter analysis. |
